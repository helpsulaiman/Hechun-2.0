...
const LeaderboardPage: React.FC = () => {
    ...
    useEffect(() => {
        if (!authLoading && allUsers !== undefined) {
            setLoading(false);
        }
    ...
    if (loading || allUsers === undefined) {
        return (
            <Layout title="Leaderboard - Hečhun" fullWidth={true}>
    ...
    let usersToDisplay: UserProfile[] = [...allUsers];
    ...
    if (!user) {
        ...
        if (guestSkillsStr) {
            ...
            if (guestSkills.reading !== undefined || guestSkills.writing !== undefined) {
                const reading_writing = Math.max(guestSkills.reading || 0, guestSkills.writing || 0);
                guestSkills.reading_writing = reading_writing;
                delete guestSkills.reading;
                delete guestSkills.writing;
                delete guestSkills.listening;
                localStorage.setItem('hechun_guest_skills', JSON.stringify(guestSkills));
            }

            const totalXP = Math.floor(
                (guestSkills.vocabulary || 0) +
                (guestSkills.reading_writing || 0) +
                ...
                <div className="max-w-4xl mx-auto px-4 py-8 pt-24 md:pt-12">
                    ...
                    <div className="space-y-3">
                        {rest.map((rUser, index) => {
                            const rank = index + 4;
                            const isCurrentUser = rUser.user_id === user?.sub || rUser.user_id === 'guest';
                    ...
                    {!user && (
                        <div className="mt-8 text-center bg-card border border-border p-6 rounded-2xl shadow-sm">
                            ...
                            <button
                                onClick={() => {
                                    const { loginWithRedirect } = require('@auth0/auth0-react');
                                    loginWithRedirect();
                                }}
                                className="inline-block px-6 py-2 bg-primary text-primary-foreground font-bold rounded-full hover:opacity-90 transition-opacity"
                            >
                                Log In / Sign Up
                            </button>
                        </div>
